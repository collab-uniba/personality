...
def get_openness(score):
    ...
    return (- 0.21*score.pronoun - 0.16*score.i - 0.1*score.we - 0.12*score.you - 0.13*score.negate - 0.11*score.assent 
...
def get_conscientiousness(score):
    ...
    return (- 0.17*score.negate - 0.18*score.negemo - 0.19*score.anger - 0.11*score.sad - 0.11*score.cogproc - 0.12*score.cause
...
def get_extraversion(score):
    ...
    #physical states
    return (0.11*score.we + 0.16*score.you - 0.12*score.number + 0.1*score.posemo - 0.09*score.cause - 0.11*score.tentat
...
def get_agreeableness(score):
    ...
    return (0.11*score.pronoun + 0.18*score.we + 0.11*score.number + 0.18*score.posemo - 0.15*score.negemo - 0.23*score.anger
...
def get_neuroticism(score):
    ...
    return (0.12*score.i - 0.15*score.you + 0.11*score.negate -0.11*score.article +0.16*score.negemo + 0.17*score.anx
...
def get_profile_liwc(session, logger):
    ...
    for score in scores:
        ...
        lpm = LiwcProjectMonth(dev_uid = score.dev_uid, project_name = score.project_name, month = score.month, email_count = score.email_count, word_count = score.wc, scores = big5)
        session.add(lpm)
        session.commit()